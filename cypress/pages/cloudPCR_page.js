...
class PCRPage {
    ...
    static turnOnServer() {
        cy.server()
        cy.route('POST', '/Account/Login?returnUrl=/Application').as("openApplication")
    ...
    static waitForOpenApp() {
        cy.wait('@openApplication')
    }
    ...
    static clickOnButton(arg) {
        cy.get(arg).click({timeout: 30000});
    }
    ...
    static fillRequiredSelectFields() {
        cy.get('i[class="fa fa-question-circle"]').parent()
            .siblings('select[required-field]').each(($el) => {
